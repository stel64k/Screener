...
def analyze_market():
    """Основная функция анализа рынка с использованием многопоточности."""
    try:
        futures_info = client.futures_exchange_info()
        symbols = [item['symbol'] for item in futures_info['symbols'] if item['quoteAsset'] == 'USDT']
        
        # Создаем ThreadPoolExecutor для многопоточной обработки
        with ThreadPoolExecutor(max_workers=10) as executor:  # Оптимальное количество потоков может быть настроено
            # Отправляем задачи в ThreadPoolExecutor
            futures = {executor.submit(analyze_symbol, symbol): symbol for symbol in symbols}
            
            # Используем as_completed для обработки завершения задач
            for future in tqdm(as_completed(futures), total=len(futures), desc="Анализ рынка"):
...
def main():
    ...
    while True:
        ...
        time.sleep(1)  # Ожидание 3 минуты
